...
interface MangaDetailsModalProps {
  manga: Manga;
  abaPrincipal: "MANGA" | "ANIME"; // ✅ Identificador de Aba
  aoFechar: () => void;
...
export default function MangaDetailsModal({ manga, abaPrincipal, aoFechar, aoAtualizarCapitulo, aoAtualizarDados, aoDeletar }: MangaDetailsModalProps) {
  ...
  // Tradução do termo conforme a aba
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-[#0e0e11] w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[3rem] border border-zinc-800 shadow-2xl custom-scrollbar">
        ...
        {/* Banner Superior com Capa e Título */}
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          {/* Coluna 1: Controles de Progresso */}
          <div className="space-y-8">
            ...
            <div className="bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800">
              ...
              >
                {["Lendo", "Completos", "Planejo Ler", "Pausados", "Dropados"].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
          </div>

          {/* Coluna 2 e 3: Sinopse e Comentários */}
          <div className="md:col-span-2 space-y-8">
            ...
            <div>
              <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-4">Anotações do Hunter</p>
              <textarea 
                className="w-full h-32 bg-zinc-900/50 border border-zinc-800 p-6 rounded-3xl text-sm text-zinc-300 outline-none focus:border-zinc-500 transition-all resize-none"
