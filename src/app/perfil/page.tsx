...
// =============================================================================
// [SESSÃO 2] - CATÁLOGO DA LOJA (MISSÕES ALIMENTAM A LOJA)
// =============================================================================
const LOJA_ITENS = [
...
export default function PerfilPage() {
  ...
  async function carregarDados() {
    ...
    if (m || a || f || l) {
      ...
      setStats({
        ...
        horasVida: Math.floor(((epsVistos * 23) + minFilmes) / 60), favs: all.filter(o => o.favorito).length,
        filmes: (f || []).length, livros: (l || []).length
      });
      const t = all.length;
      if (t >= 1000) setElo({ tier: "DIVINDADE", cor: "from-white via-cyan-200 to-white", glow: "shadow-white/60 shadow-[0_0_40px_white]", efeito: "animate-pulse" });
  ...
  async function atualizarPerfil() {
    setSalvando(true);
    await supabase.from("perfis").update({ nome_exibicao: dadosPerfil.nome, avatar: dadosPerfil.avatar, bio: dadosPerfil.bio, cor_tema: dadosPerfil.tema, custom_color: dadosPerfil.custom_color, pin: dadosPerfil.pin, anilist_token: dadosPerfil.anilist_token }).eq("nome_original", usuarioAtivo);
    setSalvando(false); alert("✨ Hunter Sincronizado!");
  }

  async function fazerUploadAvatar(e: any) {
    setFazendoUpload(true);
    const file = e.target.files[0];
    const path = `${usuarioAtivo}-${Math.random()}`;
  ...
  function exportarBiblioteca() {
    const backup = { hunter: dadosPerfil.nome, biblioteca: obrasUsuario };
  ...
  if (carregando) return <div className="min-h-screen bg-[#040405] flex items-center justify-center text-white italic animate-pulse font-black">SINCRONIZANDO HUB...</div>;
  ...
  return (
    <main className="min-h-screen bg-[#040405] flex flex-col items-center justify-center p-6 relative overflow-hidden" style={{ "--aura": dadosPerfil.custom_color } as any}>
      ...
      <div className={`bg-[#0e0e11]/95 backdrop-blur-2xl rounded-[3.5rem] p-12 border border-white/5 relative flex flex-col items-center shadow-2xl transition-all duration-700 z-10 ${telaCheia ? 'w-full max-w-6xl' : 'w-full max-w-[550px]'}`}>
        {/* Layer 1: Aura do Rank | Layer 2: Moldura da Loja */}
        ...
        <div className="flex gap-8 border-b border-white/5 w-full justify-center pb-6 mb-10">
        ...
        <div className="w-full h-[320px] overflow-y-auto custom-scrollbar px-2">
          {abaAtiva === "STATUS" && (
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 bg-gradient-to-r from-zinc-900 to-black p-6 rounded-3xl border border-white/5 flex flex-col items-center group overflow-hidden">
                ...
                <a href={`/api/auth/anilist?hunter=${usuarioAtivo}`} className="mt-6 w-full py-3 bg-blue-600/10 border border-blue-500/30 text-blue-500 rounded-xl text-[9px] font-black uppercase text-center">{dadosPerfil.anilist_token ? "✅ AniList Sincronizado" : "🔗 Conectar AniList"}</a>
              </div>
            </div>
          )}
          ...
          {abaAtiva === "MISSÕES" && (
            <div className="space-y-4 pb-10">
              {[ { t: "Check-in Diário", r: 10, i: "👋" }, { t: "Leitor Assíduo", r: 20, i: "📚" }, { t: "Sétima Arte", r: 20, i: "🎬" }, { t: "Caçador Ativo", r: 25, i: "🎯" }, { t: "Curador", r: 15, i: "✨" } ].map((m, i) => (
                <div key={i} className={`p-5 rounded-3xl border flex items-center justify-between transition-all ${missoesProgresso[i] ? 'bg-black/40 border-green-500/20' : condicoesMissoes[i] ? 'bg-zinc-900 border-yellow-500/40' : 'bg-zinc-900/50 border-zinc-800'}`}>
                  <div className="flex items-center gap-4"><span className="text-3xl">{m.i}</span><div><p className={`font-bold uppercase text-[10px] ${missoesProgresso[i] ? 'text-green-500' : 'text-white'}`}>{m.t}</p><p className="text-[8px] text-zinc-500 uppercase">+{m.r} Esmolas</p></div></div>
                  <button onClick={() => completarMissao(i, m.r)} disabled={missoesProgresso[i] || !condicoesMissoes[i]} className="px-4 py-2 rounded-xl text-[9px] font-black border border-white/10">{missoesProgresso[i] ? "Coletado" : "💰"}</button>
                </div>
              ))}
            </div>
          )}
          ...
          {abaAtiva === "CONFIG" && (
            <div className="space-y-6 pb-10">
              ...
              <div className="flex gap-3"><input type="text" placeholder="Avatar URL" className="flex-1 bg-black border border-white/5 p-4 rounded-xl text-white text-xs outline-none" value={dadosPerfil.avatar} onChange={e => setDadosPerfil({...dadosPerfil, avatar: e.target.value})} /><label className="px-6 py-4 rounded-xl border border-white/10 text-[10px] font-black uppercase cursor-pointer">{fazendoUpload ? "⏳" : "📁 PC"}<input type="file" className="hidden" onChange={fazerUploadAvatar} /></label></div>
            </div>
          )}
        </div>

        <div className="w-full flex flex-col gap-3 mt-8 relative z-20">
          <button onClick={exportarBiblioteca} className="py-4 rounded-xl border border-zinc-800 text-[9px] font-black uppercase text-zinc-500 hover:text-white transition-all">💾 Exportar JSON</button>
